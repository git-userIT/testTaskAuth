...
func selDataUser(userData User) (DataUser, bool) {
	...
	pswPg := os.Getenv("PGSQL_PSW") // "11111111"

	connString := fmt.Sprintf("host=%s port=%d user=%s password=%s dbname=%s sslmode=disable", host, port, user, pswPg, dbname)
...
func chUserExist(userData User) bool {
	...
	connStr := fmt.Sprintf("host=%s port=%d user=%s password=%s dbname=%s sslmode=disable", host, port, user, pswPg, dbname)
